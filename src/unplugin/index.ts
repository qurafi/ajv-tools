...
export default createUnplugin((config: PluginOptions) => {
    ...
    async function initSchemaBuilder() {
        ...
        schema_builder = await createSchemaBuilder({
            ...
            async moduleLoader(context) {
                const { files, defaultModuleLoader, config } = context;
                if (vite_server) {
    ...
    return {
        ...
        async load(raw_id) {
            ...
            if (id === resolved_prefix && queries.get("t")) {
                ...
                const propagate_queries = `?${instance}${
                    raw_code ? "&code" : ""
                }`;
            ...
            if (schema_ref.startsWith("/")) {
                ...
                if (raw_schema) {
                    ...
                    if (!schemas) {
                        throw new Error(
                            `Could not find schemas for file ${schema_path}`
                        );
                    }
            ...
            if (schema_ref.startsWith(":")) {
                ...
                const code = await schema_builder.getSchemaCode(
                    id,
                    instance,
                    interop
                );
            ...
            resolveId(source, importer, { ssr }) {
                ...
                if (raw_id != null) {
                    ...
                    if (has_client && has_server) {
                        throw new Error(
                            "Cannot have both server & client query"
                        );
                    }

                    if (has_server && !ssr) {
                        throw new Error(
                            "Using a server instance inside a client build"
                        );
                    }
