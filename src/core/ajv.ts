...
export function createAjvFileStore(opts: AjvFilesStoreOptions) {
    ...
    async function loadFileSchemas(
        ...
        if (schemas && !isObject(schemas)) {
            throw new TypeError(
                `resolveModule of ${file} returned non object value`
            );
        }
    ...
    function getSchemaFileCode(
        ...
        return transformSchemaCode(
            code + export_schema_code,
            instance,
            interop
        );
    }
